  ...
  const [playerKey, setPlayerKey] = useState<number>(0);
   ...
   const openForm = (creative: Creative | null = null) => {
     ...
     if (creative) {
       ...
       // Check if publish_date exists and is valid before parsing
   ...
   const handleSave = async (e: React.FormEvent) => {
     ...
     try {
       ...
       const dataToSave: Partial<Omit<Creative, 'id' | 'created_at' | 'updated_at' | 'campaignName'>> = {
         ...
         publish_date: formData.publish_date ? new Date(formData.publish_date).toISOString().split('.')[0]+"Z" : null, // Ensure ISO format for DB
         comments: formData.comments?.trim() === '' ? null : formData.comments, // Save null if comments are cleared
       };

        // Explicitly delete fields that should not be sent for update/create
        // delete dataToSave.id; // Let API handle ID for POST
        // delete dataToSave.created_at;
        // delete dataToSave.updated_at;
        // delete dataToSave.campaignName;
   ...
   const handleDelete = async (id: string) => {
     ...
     try {
       ...
       if(selectedCreativeDetail?.id === id){ // Close detail if deleted item is open
           closeCreativeDetail();
       }
     } catch (error: any) {
       toast({
              ...
              {['image', 'video'].includes(formData.type || '') ? (
                ...
                <>
                  ...
                  <Textarea
                    ...
                    required // Add required for text types as well
                  />
                </>
              ) : (
      ...
      <Dialog open={isDetailOpen} onOpenChange={setIsDetailOpen}>
        <DialogContent className={cn(cardStyle, "max-w-2xl md:max-w-4xl max-h-[90vh]")}>
          <DialogHeader>
            ...
            {/* Keep the existing description here as it's more specific */}
                  ...
                  <div className="mt-2 text-center">
                    ...
                    <div className="mt-2 flex justify-center gap-2">
                      ...
                      <Button
                        ...
                        variant="outline"
                        className={neumorphicButtonStyle}
                        onClick={() => { closeCreativeDetail(); openForm(selectedCreativeDetail); }} // Close detail, then open form
                      >
                ...
                <div className="flex items-center justify-between mb-1">
                  <Label className={labelStyle}>Comentários</Label>
                   <div className="flex gap-1">
                        {/* Keep only save comment button here */}
                   </div>
                </div>
                <div className="space-y-2">
                  <Textarea
